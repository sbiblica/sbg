...
class ventas_reporte(report_sxw.rml_parse):
    ...
    def lineas(self, datos):
        ...
        for l in lineas:
            ...
            l['tipo_doc'] = tipo_doc
            logging.getLogger('ventas').warn(tipo_doc)
            logging.getLogger('ventas').warn(l['total_base'])
            logging.getLogger('ventas').warn(l['total_impuesto'])
            
            if l['total_impuesto'] == 0:
                l['tipo_gasto'] = 'exento'
                l['total_base'] = l['amount_untaxed']
            ...
            if tipo_doc == 'FACT':
                if l['tipo_gasto'] != 'mixto':
                    ...
                    self.totales['servicio']['total'] += l['total_base']+iva_servicio
            elif tipo_doc == 'NC':
