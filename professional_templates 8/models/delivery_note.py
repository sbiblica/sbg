...
class DN(models.Model):
	_inherit=["stock.picking"]
        ...
        def onchange_partner_id(self, partner_id):
            res = {}
            if partner_id:
                style = self.env['res.partner'].browse(partner_id).style or self.env.user.company_id.default_style or self.env.ref('professional_templates.default_style_for_all_reports')
                res.update(dict(value=dict(dn_style = style, pk_style= style)))
            return res
        ...
        def create(self, vals):
            ...
            if res:
                style = res.partner_id.style.id or self.env.user.company_id.default_style.id or self.env.ref('professional_templates.default_style_for_all_reports').id
                res.write(dict(dn_style=style, pk_style=style))
            return res
