...
class InvoiceTemplates(models.Model):
	...
	def onchange_partner_id(self, type, partner_id, date_invoice=False, payment_term=False, partner_bank_id=False, company_id=False):
            res = super(InvoiceTemplates, self).onchange_partner_id(type, partner_id, date_invoice=date_invoice, payment_term=payment_term, partner_bank_id=partner_bank_id, company_id=company_id)
            if not partner_id:
                return res
            if res:
                style = (self.env['res.partner'].browse(partner_id).style.id or self.env.user.company_id.default_style.id or self.env.ref('professional_templates.default_style_for_all_reports').id)
        ...
        def create(self, vals):
            ...
            if partner and not vals.get('style'):
                vals['style'] = partner.style.id or self.env.user.company_id.default_style.id or self.env.ref('professional_templates.default_style_for_all_reports').id
