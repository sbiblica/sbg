...
class AccountCSVExport(orm.TransientModel):
    ...
    def action_manual_export_account(self, cr, uid, ids, context=None):
        ...
        return {
            ...
            'res_model': 'account.csv.cta.export',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': this.id,
            'views': [(False, 'form')],
    ...
    def _get_rows_journal_entries(self, cr, uid, ids,
        ...
        """,
                   {'period_ids': tuple(
                       period_range_ids), 'account_ids': tuple(account_ids)}
                   )
        while 1:
    ...
    def get_data(self, cr, uid, ids, result_type, context=None):
        ...
        if form.account_ids:
            ...
            j_obj = self.pool.get("account.account")
            account_ids = j_obj.search(cr, uid, [], context=context)
